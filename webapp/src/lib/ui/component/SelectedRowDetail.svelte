<script lang="ts">
    ...
    const updateQty  = () => {
        // Auto trigger quantity update after idling for X ms
        if ( !selectedItem ){
            return
        }
        clearTimeout(qtyTimeout);
        qtyTimeout = setTimeout(() => {
            qtyUpdateForm?.requestSubmit();
        }, 250);
    }
...
<TableBodyRow class={openRow ? "bg-gray-50 dark:bg-gray-50 border-gray-200 border-y" : "hidden"}>
    <TableBodyCell class="p-0">
     ...
     <TableBodyCell class="p-0 flex items-start">
        <div transition:slide={{ duration: 300, axis: "y" }}>
            {#if openRow && !selectedItem.barcodeControlled }
                <form bind:this={qtyUpdateForm} method="POST" action="?/updateQty" use:enhance={onSubmitUpdateQty}>
                <input type="hidden" name="id" value={selectedItem!.id} />
                <Label for="updateQty" class="mb-1 text-sm text-gray-900 dark:text-white text-center">Quantity</Label>
